         <div class="col-sm-7 m-auto">
             <div class="border p-3 bg-white">
               <div class="chaperone-add">
                ...
                <form action="<?php echo site_url('subadmin/chaperone_update');?>" enctype="multipart/form-data" method="post" data-parsley-validate="" name="myform">
                    <input type="hidden" name="chaperone_id" value="<?= $chaperoneDetail->id;?>">
                    <div class="row">
                         <div class="col-sm-12 ">
                             <div class="form-group">
                                <div class="row align-items-center">
                                     <label class="col-sm-3"><?= $this->lang->line('chaperone_number'); ?> </label>
                                     <div class="col-sm-9">
                                         <input class="form-control" type="text" name="f_name" placeholder="<?= $this->lang->line('chaperone_unique_id'); ?> " value="<?= $chaperoneDetail->chaperone_unique_id;?>" readonly>
                                     </div>
                                </div>  
                             </div>
                             <?php 
                                $names = explode(' ', $chaperoneDetail->chaperone_name, 2);
                                $f_name = $names[0];
                                // $family_name = $names[1];
                                if(!empty($names[1])){
                                     $family_name = $names[1];
                             ...
                             <!--               <option data-tokens="<?php echo $key['id']; ?>" value="<?php echo $key['id']; ?>" <?php echo $key['id'] == $chaperoneDetail->bus_id ? 'selected' : '' ?>><?php echo $key['bus_number']; ?></option>-->
                             <!--            <?php } ?>-->
                             <!--       </select>-->
                             <!--        </div>-->
                             <!--   </div>  -->
                             <!--</div>-->
                              ...
                              <div class="form-group">
                                <div class="row align-items-center">
                                     ...
                                     <div class="col-sm-5">
                                         <input class="form-control" id="secret_code" type="text" placeholder="<?php echo $this->lang->line('secret_code'); ?>" name="secret_code" value= "<?= $chaperoneDetail->secret_code;?>" required data-parsley-required data-parsley-required-message="<?php echo $this->lang->line('enter_secret_code'); ?>">
                                     </div>
                             ...
                             <div class="form-group">
                                <div class="row align-items-center">
                                     ...
                                     <div class="col-sm-9">
                                         <!-- <input class="form-control" type="text" name="note"  placeholder="<?php echo $this->lang->line('note'); ?>" required data-parsley-required-message="<?php echo $this->lang->line('enter_note'); ?>" value="<?= $chaperoneDetail->note;?>"> -->
                                         <input class="form-control" type="text" name="note"  placeholder="<?php echo $this->lang->line('note'); ?>" value="<?= $chaperoneDetail->note;?>">
                                     </div>
                                </div>  
                             </div>
                         </div>
...
<script type="text/javascript">
    ...
    function check_chaperoneNumber(chaperone_phone_number)
    {
      // alert('hi');
        // alert(bus_plate_number);
        if(chaperone_phone_number != ''){

             $.ajax({
                url: '<?php echo site_url("subadmin/ChaperoneController/check_chaperoneNumber"); ?>',
                type: "POST",
                data: {
                    "chaperone_phone_number" : chaperone_phone_number
                },
                success: function (response) 
                {
                    ...
                    if (response == '0') {
                        ...
                        $('#errmsg_phoneno').html("Chaperone number already exit");
                        $('#chaperone_phone_number').val('');
                    }
                }
            });
        }
